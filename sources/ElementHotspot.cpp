...
void ElementHotspot::Clicked(DisplayDirector* director)
{
	...
	String action = stylesheet->PropertyForElement("action", element->TagName());
	...
	String link = stylesheet->PropertyForElement("link", element->TagName());
	if (!link.empty()) {
		// chase down "attr()"
		if (link.startsWith("attr(")) {
			String attrName = link.substr(5, link.length() - 6);
